...
Game.prototype._GetCollidingEntities = function(){
  ...
  obstacles = this._AddDoorsToObstaclesIfTheyAreClosed(doors, obstacles);
...
Game.prototype._AddDoorsToObstaclesIfTheyAreClosed = function(doors, obstacles){
  doors.forEach(function (door, door_direction, doors_map) {
    if (door.open == false){
      obstacles.push(door);
    };
  });
...
function Level(){
  ...
  this.rooms = this._CreateRandomRooms();
};

Level.prototype._CreateRandomRooms = function(){
  const grid_size = 5;
  const max_rooms = 3;
...
Level.prototype.CheckRoomVictoryCondition = function(){ //refactor!
  if (this.current_room.IsVictoryConditionFulfilled()){
    
    var adjacent_room_existence = this.rooms.IsCurrentRoomAdjacentToRooms();
    
    if (adjacent_room_existence.get("top_room")){
      this.current_room.OpenDoor("top");
    };
    if (adjacent_room_existence.get("left_room")){
      this.current_room.OpenDoor("left");
    };
    if (adjacent_room_existence.get("bottom_room")){
      this.current_room.OpenDoor("bottom");
    };
    if (adjacent_room_existence.get("right_room")){
      this.current_room.OpenDoor("right");
    };
  };
};
...
Level.prototype.ChangeCurrentRoomAndCharacterPosition = function(character, collision_flags){ //refactor!
  ...
  if (adjacent_room_existence.get("top_room") && collision_flags.get("char_door_top")){
    ...
    this._ChangeCharacterPositionOnRoomTransition(character, "down");
  }
    ...
    this._ChangeCharacterPositionOnRoomTransition(character, "right");
  }
  else if (adjacent_room_existence.get("bottom_room") && collision_flags.get("char_door_bottom")){
    this._SetCurrentRoom(new MatrixPosition(position.row + 1, position.column));
    this._ChangeCharacterPositionOnRoomTransition(character, "up");
  }
  else if (adjacent_room_existence.get("right_room") && collision_flags.get("char_door_right")){
    this._SetCurrentRoom(new MatrixPosition(position.row, position.column + 1));
    this._ChangeCharacterPositionOnRoomTransition(character, "left");
  };
};

Level.prototype._ChangeCharacterPositionOnRoomTransition = function(character, location){ //refactor!
  if (location == "up"){
    character.center.y = 2 * character.radius;
  }
  else if (location == "left"){
    character.center.x = 2 * character.radius;
  }
  else if (location == "down"){
    character.center.y = canvas.height - 2 * character.radius;
  }
  else if (location == "right"){
    character.center.x = canvas.width - 2 * character.radius;
  }
};
...
Room.prototype.OpenDoor = function(side){
  var door = this.doors.get(side);
...
StartingRoom.prototype.IsVictoryConditionFulfilled = function(){
  return true;
};
...
MiddleWallRoom.prototype.IsVictoryConditionFulfilled = function(){
  return true;
};
...
SideWallRoom.prototype.IsVictoryConditionFulfilled = function(){
  return true;
};
