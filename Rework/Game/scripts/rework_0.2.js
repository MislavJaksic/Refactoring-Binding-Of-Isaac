...
{  
	...
	level: function(){
		...
		if(d_pressed && x_character < canvas.width-character_radius && (y_character < canvas.height/2+30 && y_character > canvas.height/2-30)) {
			x_character += 5;
		}
		...
		if(a_pressed && x_character > 0+character_radius && (y_character < canvas.height/2+30 && y_character > canvas.height/2-30)) {
			x_character -= 5;
		}
		...
		if(w_pressed && y_character > 0+character_radius && (x_character < canvas.width/2+30 && x_character > canvas.width/2-30)) {
			y_character -= 5;
		}
		...
		if(s_pressed && y_character < canvas.height-character_radius && (x_character < canvas.width/2+30 && x_character > canvas.width/2-30)) {
			y_character += 5;
		}
...
if ( y_character == character_radius && (x_character < canvas.width/2+30 && x_character > canvas.width/2-30)){
door_number = 3;
...
if ((y_character < canvas.height/2+30 && y_character > canvas.height/2-30) && x_character == canvas.width-character_radius){
door_number = 4;
randomly_generated_door=Math.floor(Math.random()*3+1);
room_counter++;
...
if ( y_character == canvas.height-character_radius && (x_character < canvas.width/2+30 && x_character > canvas.width/2-30)){
...
if ( (y_character < canvas.height/2+30 && y_character > canvas.height/2-30) && x_character == character_radius){
door_number = 2;
randomly_generated_door=Math.floor(Math.random()*3+1);
room_counter++;
...
function DrawHearts(){
  health_bar_start_point = new Point(10, 10);
  heart_size = 40;
  heart_spacing = heart_size / 4;
  health = character_health;
  
  test_health_bar = new HealthBar(health_bar_start_point, heart_size, heart_spacing, health);
  test_health_bar.DrawShape();
};
...
function DrawCharacter() {
  ...
  character = new Circle(character_position, character_radius);
...
function Rectangle(start_point, end_point){
  Shape.call(this);
...
Rectangle.prototype.DrawShape = function(){
  ...
  context.rect(this.start_point.x, this.start_point.y, this.end_point.x, this.end_point.y);
...
function Wall(start_point, thickness, length, orientation){
  end_point = ChooseWallEndPoint(thickness, length, orientation);
  
  Rectangle.call(this, start_point, end_point);
};

Wall.prototype = Object.create(Rectangle.prototype);
Wall.prototype.constructor = Wall;

function ChooseWallEndPoint(thickness, length, orientation){
  if (orientation === "horizontal"){
    end_point = new Point(length, thickness);
  }
  else if (orientation === "vertical"){
    end_point = new Point(thickness, length);
  }
  else{
...
function Door(start_point, thickness, length, orientation){
  Wall.call(this, start_point, thickness, length, orientation)
};
...
function HealthBar(health_bar_start_point, token_size, token_spacing, health){
  this.health_bar_start_point = health_bar_start_point;
...
HealthBar.prototype.DrawShape = function(){
  DrawHealthBarFullHearts(this.health_bar_start_point, this.number_of_full_hearts, this.token_size, this.token_spacing);

  if (this.number_of_left_half_hearts === 1){
    DrawHealthBarLeftHalfHeart(this.health_bar_start_point, this.number_of_full_hearts, this.token_size, this.token_spacing);
  };
};
...
function HalfHeart(start_point, size, heart_part){
  point_array = CreateFourHalfHeartPoints(start_point, size, heart_part);
...
function CreateFourHalfHeartPoints(start_point, size, heart_part){
